...
class BoltzmannSolver(object):
    ...
    def _linsystem(self, F):
        ...
        q_sigma2 = self.OmegaN ** 2 / (sigma_tilde ** 2 * GAMMA ** 2 * self.benergy)
        # The R (G) term, which we add to A.
        G = 2 * self.denergy32 * nu / 3

        A = self._scharf_gummel(sigma_tilde, G, q_sigma2)
    ...
    def _scharf_gummel(self, sigma_tilde, G=0, q_sigma = 0):
        D = self.DA / (sigma_tilde * (1 + q_sigma)) + self.DB
